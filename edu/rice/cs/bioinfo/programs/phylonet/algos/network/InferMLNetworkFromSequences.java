...
import edu.rice.cs.bioinfo.programs.phylonet.algos.integration.FullLikelihoodFromSequenceBackup2;
...
/**
 ...
 public class InferMLNetworkFromSequences extends MDCOnNetworkYFFromRichNewickJung {
     ...
     public List<Tuple<Network,Double>> inferNetwork(String[] gtTaxa, List<char[][]> sequences, Map<String,List<String>> species2alleles, GTRSubstitutionModel gtrsm, double theta, int maxReticulations, int numSol){
         ...
         */
         ...
         for(int i=0; i<_numRuns; i++) {
             FullLikelihoodFromSequence likelihoodCalculator = new FullLikelihoodFromSequence(allTrees, gtTaxa, distinctSequences, gtrsm, theta, 0, _maxHeight);
             //TODO
             likelihoodCalculator.setRandomSeed(_seed);

             likelihoodCalculator.setParallel(_numThreads);
             likelihoodCalculator.computeGTLikelhoodWithIntegral(_numSamples, _numBins);
             ...
             Func1<DirectedGraphToGraphAdapter<String, PhyloEdge<String>>, Double> scorer = getScoreFunction(likelihoodCalculator, species2alleles);
     ...
     private Func1<DirectedGraphToGraphAdapter<String,PhyloEdge<String>>, Double> getScoreFunction(final FullLikelihoodFromSequence likelihoodCalculator, final Map<String, List<String>> species2alleles){
         return new Func1<DirectedGraphToGraphAdapter<String,PhyloEdge<String>>, Double>() {
             public Double execute(DirectedGraphToGraphAdapter<String,PhyloEdge<String>> network) {
                 ...
                 double score = findUltrametricOptimalBranchLength(speciesNetwork, species2alleles, likelihoodCalculator);
     ...
     private double findUltrametricOptimalBranchLength(final Network<Object> speciesNetwork, final Map<String, List<String>> species2alleles, final FullLikelihoodFromSequence likelihoodCalculator){
         ...
         double initialProb = computeProbability(speciesNetwork, species2alleles, likelihoodCalculator);
             ...
             for(final NetNode<Object> child : speciesNetwork.getNetworkNodes()) // find every hybrid node
             {
                 ...
                 UnivariateFunction functionToOptimize = new UnivariateFunction() {
                     public double value(double suggestedProb) {
                         ...
                         double lnProb = computeProbability(speciesNetwork, species2alleles, likelihoodCalculator);
     ...
     protected double computeProbability(Network speciesNetwork, Map<String, List<String>> species2alleles, FullLikelihoodFromSequence likelihoodCalculator) {
         double totalProb = likelihoodCalculator.computeNetworkLikelhoodWithIntegral(speciesNetwork, species2alleles);
         //TODO
         //System.out.println(network2String(speciesNetwork) + ": " + totalProb);
         return totalProb;
     }
     ...
     public static void main(String[] args){
         ...
         InferMLNetworkFromSequencesBackup inference = new InferMLNetworkFromSequencesBackup();
