...
public class GeneTreeProbabilityDQ {
    ...
    private List<Double> _xlg = new ArrayList<>();

    public List<Double> getXLG() {
        return _xlg;
    }
    ...
    public List<Double> calculateGTDistribution(Network<Double> net, List<Tree> gts, Map<String,String> allele2species, boolean toPrint){
        ...
        //TODO mulTreeMatrix is for Dingqiao
        boolean[][] mulTreeMatrix = new boolean[_S.length][_S[0].length];
        for(int i=0; i<_S.length; i++){
            for(int j=0; j<_S[0].length; j++){
                if(i == j){
                    mulTreeMatrix[i][j] = true;
                }
                else{
                    mulTreeMatrix[i][j] = _S[i][j];
                }
            }
        }
        ...
        for(Tree gt: gts){
            ...
            double gtprob = 0;
            do{
                //TODO: alleleMapping is for dingqiao's data
                Map<String,String> alleleMapping = new HashMap<>();
                ...
                for(int i=0; i<_netTaxa.size(); i++){
                    ...
                    for(int j=0; j<alleles.size(); j++){
                        ...
                        alleleMapping.put(alleles.get(j), baseName+"_"+subscribes[j]);
                    }
                }
                ...
                for(int[] history: histories){
                    ...
                    //TODO: coalHistoryMapping is for dingqiao's data
                    Map<TNode, Integer> coalHistoryMapping = new HashMap<>();
                    for(int j=0; j<history.length; j++){
                        if(history[j] != -1){
                            coalHistoryMapping.put(gt.getNode(j), history[j]);
                        }
                    }
                    if(_printDetails) {
                    ...
                    MDCOnNetworkForCoalHistory mdc = new MDCOnNetworkForCoalHistory();
                    int xl = mdc.countExtraCoal(_mulTree, gt, alleleMapping, _tname2nname, coalHistoryMapping, mulTreeMatrix);
                    //System.out.println("Total number of xl: " + xl);
                    double coalProb = Double.parseDouble(computeProbability(mapping, history, false));
                    gtmapprob += coalProb;
            ...
            _xlg.add(xlg/totalprob);
//            System.out.println(xlg);
     ...
     * @return	a list of probabilities corresponding to the list of gene trees.
     */
    public MutableTuple<List<Double>, List<Double>> calculateExpectXL(Network<Double> net, List<Tree> gts, Map<String,String> allele2species){
        networkToTree(net);
        ...
        List<Double> xlgList = new ArrayList<Double>();
        List<Double> gtpList = new ArrayList<>();
        for(Tree gt: gts){
            if(_printDetails){
                System.out.println("Gene tree " + gt+" :");
            }
            ...
            xlgList.add(expectedXL);
            gtpList.add(gtprob);
            // expectedXL = expectedXL / gtprob;
        }

        return new MutableTuple<List<Double>, List<Double>> (xlgList, gtpList);
    }
