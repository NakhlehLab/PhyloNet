...
/*
 *@ClassName: treeAugmentInferNetworkMDC
 *@Description
 *@Author: Zhen Cao
...
import edu.rice.cs.bioinfo.programs.phylonet.algos.search.HillClimbing.SimpleHillClimbingTA;
...
public class treeAugmentInferNetworkMDC extends InferNetworkMP {
    ...
    public List<Tuple<Network,Double>> inferNetwork(List<MutableTuple<Tree,Double>> gts, Map<String,List<String>> species2alleles, int maxReticulations, int numSol){
        ...
        NetworkRandomNeighbourGenerator allNeighboursStrategy = new NetworkRandomNeighbourGenerator(new NetworkRandomTopologyNeighborGeneratorTA(speciesNetwork, _operationWeights, maxReticulations, _moveDiameter, _reticulationDiameter, _fixedHybrid, _seed), 1, new NonUltrametricNetworkRandomParameterNeighbourGenerator(), 0, _seed);
        Comparator<Double> comparator = getDoubleScoreComparator();
        SimpleHillClimbingTA searcher = new SimpleHillClimbingTA(comparator, allNeighboursStrategy);
    ...
    private void computeXLParallel(Network network, List<MutableTuple<Tree,Double>> gts, Map<String, List<String>> species2alleles, int[] xls){
        ...
        for(int i=0; i<_numProcessors; i++){
            myThreads[i] = new treeAugmentInferNetworkMDC.MyThread(network, gts, species2alleles, xls, mdc);
            myThreads[i].start();
        }
