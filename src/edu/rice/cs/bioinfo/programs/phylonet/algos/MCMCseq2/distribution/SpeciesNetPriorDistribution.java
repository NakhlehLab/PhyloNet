package edu.rice.cs.bioinfo.programs.phylonet.algos.MCMCseq2.distribution;


import edu.rice.cs.bioinfo.library.programming.extensions.java.lang.iterable.IterableHelp;
import edu.rice.cs.bioinfo.programs.phylonet.algos.MCMCseq2.structs.NetNodeInfo;
import edu.rice.cs.bioinfo.programs.phylonet.algos.MCMCseq2.structs.PopulationSize;
...
public class SpeciesNetPriorDistribution {
    ...
    public double logPrior(Network<NetNodeInfo> net, List<UltrametricTree> genetrees) {
        ...
        if(Utils.DEBUG_MODE){
            System.out.println("prior:"+getTopPrior(net) +","+ getDiametersPrior(net)
                    +","+ getNodeHeightsPrior(net)
                    + ","+getInheritanceProbPrior(net)
                    + ","+getPopSizePrior(net)
                    + ","+getMutationRatePrior(genetrees));
        }
