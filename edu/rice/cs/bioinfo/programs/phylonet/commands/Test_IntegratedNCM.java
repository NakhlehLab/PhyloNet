 ...
 * Date: 1/10/18
 *
 * This class is to test functionality needed for the no-common-mechanism gene tree likelihood calculation
 */
@CommandName("test_integratedNCM")
public class Test_IntegratedNCM extends CommandBaseFileOut{
    ...
    private double _blParam = Double.POSITIVE_INFINITY;


    private List<NetworkNonEmpty> Ts = new ArrayList<>();
    private NetworkNonEmpty N;


    public Test_IntegratedNCM(SyntaxCommand motivatingCommand, ArrayList<Parameter> params,
    ...
    protected boolean checkParamsForCommand(){
        ...
        ParameterIdentList geneTreeList = this.assertParameterIdentList(0);
        for (String ident: geneTreeList.Elements) {
            this.assertNetworkExists(ident, geneTreeList.getLine(), geneTreeList.getColumn());
            Ts.add(this.sourceIdentToNetwork.get(ident));
        }
        //T = this.assertAndGetNetwork(0);
        N = this.assertAndGetNetwork(1);
        ...
        if(blParam.ContainsSwitch)
        {
            if(blParam.PostSwitchParam != null)
            {
                try
                {
                    _blParam = Double.parseDouble(blParam.PostSwitchValue);
                }
                catch(NumberFormatException e)
                {
                    errorDetected.execute("Unrecognized branch length parameter value " + blParam.PostSwitchValue, blParam.PostSwitchParam.getLine(), blParam.PostSwitchParam.getColumn());
                }
            }
            else
            {
                errorDetected.execute("Expected value after switch -bl.", blParam.SwitchParam.getLine(), blParam.SwitchParam.getColumn());
            }
        }
...
//        }
    ...
    protected String produceResult() {
        ...
        Network speciesNetwork = transformer.makeNetwork(N);

        List<Tree> treeList = new ArrayList<>();
        for (NetworkNonEmpty T: Ts) {
            boolean rooted = true;
            if(T.RootageQualifier instanceof RootageQualifierNonEmpty)
                rooted = ((RootageQualifierNonEmpty)(T.RootageQualifier)).isRooted();
            String phylonetGeneTree = NetworkTransformer.toENewickTree(T);
            NewickReader nr = new NewickReader(new StringReader(phylonetGeneTree));
            STITree<Double> geneTree = new STITree<Double>(rooted);
            try {
                nr.readTree(geneTree);
            } catch (Exception e) {
                errorDetected.execute(e.getMessage(),
                        this._motivatingCommand.getLine(), this._motivatingCommand.getColumn());
            }

            treeList.add(geneTree);
        }

        // use NCM likelihood
        GeneTreeProbabilityIntegrated p = new GeneTreeProbabilityIntegrated();
        return (p.calculateGTDistribution(speciesNetwork, treeList, null, _blParam, true)).toString();


//        // use standard likelihood
//        GeneTreeProbability gtp = new GeneTreeProbability();
//        //gtp.setParallel(false);
//        //gtp.preProcess(speciesNetwork, treeList, true);
//        //double[] results = new double[treeList.size()];
//        List<Double> results;
//        results = gtp.calculateGTDistribution(speciesNetwork, treeList, null, true);
//        return results.toString();

//        // use YF likelihood
//        GeneTreeProbabilityYF gtpyf = new GeneTreeProbabilityYF();
//        gtpyf.setParallel(false);
//        gtpyf.preProcess(speciesNetwork, treeList, true);
//        double[] resultprobs = new double[treeList.size()];
//        gtpyf.calculateGTDistribution(speciesNetwork, treeList, null, resultprobs);
//        String out = "";
//        double total = 0;
//        for (double d: resultprobs) {
//            out += "\n" + Double.toString(d);
//            total += d;
//        }
//        return out + ", total = " + Double.toString(total);
