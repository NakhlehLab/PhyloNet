...
public class AssertTaxaCount extends CommandBase
{
    ...
    private int _expectedTaxaCount = -1;

    public AssertTaxaCount(SyntaxCommand motivatingCommand, ArrayList<Parameter> params, Map<String, NetworkNonEmpty> sourceIdentToNetwork, Proc3<String, Integer, Integer> errorDetected) {
    ...
    protected boolean checkParamsForCommand() {
        ...
        if(expectedRetNodeCount != null)
        {
            try
            {
                _expectedTaxaCount = Integer.parseInt(expectedRetNodeCount.Content);
            }
    ...
    protected void executeCommandHelp(Proc1<String> displayResult) throws IOException {
        ...
        for(NetworkNonEmpty net : _nets)
        {
            ...
            int foundTaxaCount = 0;
            for(NetNode node : pnet.bfs())
            {
                if(node.isLeaf())
                {
                    foundTaxaCount++;
            ...
            if(foundTaxaCount != _expectedTaxaCount)
                throw new RuntimeException("Expected " + _expectedTaxaCount + " taxa but found " + foundTaxaCount);
        }
    }
}
