 ...
 */
public class PAUPAvgMinComparator extends PAUPWindowComparator {
	...
	// constructors
	public PAUPAvgMinComparator(File paup_path, int pop_size, int num_iterations, int num_levels, DistanceMeasure dm) throws FileNotFoundException {
	...
	//		return Double.NaN;
		...
		double[] left_min_dists = new double[last_left_tree+1];
		double[] right_min_dists = new double[last_right_tree+1];

		Arrays.fill(left_min_dists, Double.MAX_VALUE);
		Arrays.fill(right_min_dists, Double.MAX_VALUE);
		...
		for(int i = 0; i <= last_left_tree; i++) {
			for(int j = 0; j <= last_right_tree; j++) {
				...
				if(dist < left_min_dists[i]) {
					left_min_dists[i] = dist;
				}

				if(dist < right_min_dists[j]) {
					right_min_dists[j] = dist;
				}
			}
		}

		double left_avg = 0;
		for(int i = 0; i < left_min_dists.length; i++) {
			left_avg += left_min_dists[i];
		}
		left_avg = left_avg / ((double) left_min_dists.length);

		double right_avg = 0;
		for(int i = 0; i < right_min_dists.length; i++) {
			right_avg += right_min_dists[i];
		}
		right_avg = right_avg / ((double) right_min_dists.length);
