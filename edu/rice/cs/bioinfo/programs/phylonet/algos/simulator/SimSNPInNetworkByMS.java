...
/**
 * Created by Jiafan Zhu
 * Date: 11/25/16
 * Time: 1:35 PM
 *
 * This class is for using ms (an external software package) to simulate SNPs of a given species networks under infinite site model
 * Note that the network needs to be ultrametric
 */
public class SimSNPInNetworkByMS {
     ...
     * @param numGTs            the number of samples need to be generated
     * @param MSPath            local ms path
     *
     * @return   the list of simulated gene trees
     */
    public List<Map<String, String>> generateSNPs(Network network, Map<String, List<String>> species2alleles, int numGTs, String MSPath){
        ...
        List<Map<String, String>> alns = new ArrayList<>();
        try{
            ...
            while((line=stdout.readLine())!=null){
                ...
                if(line.startsWith("//")){
                    ...
                    for(int i = 1 ; i <= MSName2AlleleName.size() ; i++) {
                        line = stdout.readLine();
                        if(line.length() == 0)
                            break;
                        aln.put(MSName2AlleleName.get(Integer.toString(i)), line);
                    }
                    if(aln.size() == MSName2AlleleName.size())
                        alns.add(aln);
                }
    ...
    public String generateMSCommand(Network net, Map<String, List<String>> species2alleles, int numGTs, double epsilon, Map<String,String> MSName2AlleleName){
        ...
        outputCommand = " " + totalAlleles + " " + numGTs + " -t 1.0 -I " + numPopulations + outputCommand;
    ...
    public static void main(String[] args) {
        SimSNPInNetworkByMS simulator = new SimSNPInNetworkByMS();
        simulator.generateSNPs(Networks.readNetwork("((B:0.5)X#H1:1.5::0.5,((X#H1:0.5::0.5,A:1)n1:0.5,C:1.5)n2:0.5)root;"), null, 100, "/Users/zhujiafan/Documents/Luay/msdir/ms");
    }
